...
namespace DES
{
    public partial class Form1 : Form
    {
        ...
        private void buttonEncrypt_Click(object sender, EventArgs e)
        {
            ...
            {
                string connectionString = @"Data Source=.\SQLEXPRESS;Initial Catalog=usersdb;Integrated Security=True";
                string sqlExpression = "insert into Database.keys(key, string) values('"+this.textBoxEncodeKeyWord.Text+"', '"+this.textBoxResult.Text+"'); ";

                string s = "";

                string key = textBoxEncodeKeyWord.Text;

                s = textBoxResult.Text;
                ...
                sqlExpression = "UPDATE Keys SET enc_string = " + key + " WHERE ID = " + countrows();
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    connection.Open();
                    SqlCommand command = new SqlCommand(sqlExpression, connection);
                ...
                sqlExpression = "UPDATE Keys SET enc_string = "+result+" WHERE ID = " + countrows();
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    connection.Open();
                    SqlCommand command = new SqlCommand(sqlExpression, connection);
                ...
                textBoxResult.Text = (StringFromBinaryToNormalFormat(result));
        ...
        {
            string stmt = "SELECT COUNT(*) FROM dbo.Keys";
            int count = 0;

            using (SqlConnection thisConnection = new SqlConnection(@"Data Source=.\SQLEXPRESS;Initial Catalog=usersdb;Integrated Security=True"))
            {
